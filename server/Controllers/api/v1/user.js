...
module.exports.signUp = (req, res) => {
	...
	if (
		generalUtility.isNullOrUndefined([firstName, lastName, email, password])
	) {
	...
	const hash = crypto
		...
		try{let user=await User.create({ firstName, lastName, hash, salt, email });
		await user.save();}
		catch(error)
		{
			console.log(chalk.redBright(error));
			return generalUtility.response(res, StatusCodes.INTERNAL_SERVER_ERROR, getReasonPhrase(StatusCodes.INTERNAL_SERVER_ERROR), false)
		}
